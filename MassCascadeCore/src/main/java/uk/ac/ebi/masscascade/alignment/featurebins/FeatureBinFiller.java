...
/**
 * Class to reverse fill gaps in cross-sample feature alignments. Gaps are looked up in the complementary scan
 * container, taking time shifts into account if provided.
 */
public class FeatureBinFiller {
     ...
     * @param defaultIntensity the default intensity to be set when no signal can be found in the scan container
     */
    public FeatureBinFiller(double deltaRt, double deltaPpm, double defaultIntensity) {
    ...
    /**
     * Sets optional shifts to compensate for time shifts on the feature level, e.g., due to previous feature
     * alignment.
     *
    ...
    /**
     * Reverse fills a gap for a cross-sample feature bin at a given index.
     *
     * @param index      the index to be filled
     * @param container  the corresponding scan container
     * @param bins       the total list of feature bins
     * @param binIndices the list of bin indices pointing to the feature bins to be filled
     */
    public void reverseFill(int index, ScanContainer container, List<FeatureBin> bins, Collection<Integer> binIndices) {
