...
/**
 * A factory class for constructing {@link Container} and {@link Container}
...
public class ContainerFactory {
     ...
     *                          stored.
     * @see #ContainerFactory(int)
     */
    public ContainerFactory(InterfaceProvider interfaceProvider, int n) {
     ...
     */
    public ContainerFactory(int n) {
        this(new DefaultInterfaceProvider(), n);
    }

    /**
     * Inspects whether the provided class is a {@link Container} interface.
     *
     * @param c class to test
     * @return whether the class is a CDK interface
     */
    private boolean isContainerInterface(Class<?> c) {
        return Container.class.isAssignableFrom(c) && c.isInterface();
    }
     ...
     * @see #register(ContainerFactory.ConstructorKey,
     *      ContainerFactory.Creator)
     */
    public <T extends Container> boolean register(Class<? extends T> impl) {
        ...
        for (Class<?> c : interfaceProvider.getInterfaces(impl)) {
            ...
            if (isContainerInterface(c)) {
                @SuppressWarnings("unchecked") Class<T> intf = (Class<T>) c;
     ...
     */
    public <T extends Container> boolean register(Class<T> intf, Class<? extends T> impl) {
     ...
     * @return whether registration was successful
     */
    public <S extends Container, T extends S> boolean register(Class<S> intf, Class<T> impl,
        ...
        for (Constructor<?> untyped : impl.getConstructors()) {
            @SuppressWarnings("unchecked") Constructor<T> typed = (Constructor<T>) untyped;
            registered = register(intf, typed, modifier) || registered;
        }

        if (registered) {
            logger.debug(
                    "registered '" + intf.getSimpleName() + "' with '" + impl.getSimpleName() + "' implementation");
        } else {
     ...
     */
    public <S extends Container, T extends S> boolean register(Class<S> intf, Constructor<T> constructor) {
     ...
     *                                  constructed
     */
    public <T extends Container> T ofClass(Class<T> intf, Object... objects) {

        try {
            Creator<T> constructor = get(new ObjectBasedKey(intf, objects));
            return constructor.create(objects);
        } catch (InstantiationException e) {
            throw new IllegalArgumentException("unable to instantiate container object: ", e);
     ...
     *                                  constructed
     */
    public <T extends Container> T ofClass(Class<T> intf) {
     ...
     */
    public <T extends Container> Set<Class<?>> implementorsOf(Class<T> intf) {
    ...
    private static class ModifiedCreator<T> implements Creator<T> {
        ...
        @Override
        public T create(
                Object[] objects) throws InvocationTargetException, IllegalAccessException, InstantiationException {
