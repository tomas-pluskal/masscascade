...
/**
 * Class implementing a mass spectrometry feature.
 */
public class FeatureImpl implements Feature {
    ...
    /**
     * Constructs a mass spectrometry feature.
     *
     ...
     */
    public FeatureImpl(int id, XYPoint mzIntDp, double retentionTime, Range mzRange) {
     ...
     * @param mzRange   the mz range
     */
    public FeatureImpl(int id, XYZPoint dataPoint, Range mzRange) {
     ...
     */
    public FeatureImpl(int id, XYZPoint dataPoint, Range mzRange, PropertyManager manager) {
     ...
     */
    public FeatureImpl(int id, XYZPoint dataPoint, Range mzRange, PropertyManager manager, MsnManager msnManager) {
    ...
    public Feature copy() {
        ...
        return new FeatureImpl(id, dp, new ExtendableRange(dp.y), propertyManager, msnManager);
    }
     ...
     */
    @Override
    public Feature copy(double rt) {
        XYZPoint dp = data.get(0);
        return new FeatureImpl(id, new XYZPoint(rt, dp.y, Constants.MIN_ABUNDANCE), new ExtendableRange(dp.y),
    ...
    /**
     * Adds a single point to the feature and its trace.
     *
     * @param mzIntDp the mz int data pair
     * @param rt      the retention time
     */
    @Override
    public void addFeaturePoint(XYPoint mzIntDp, double rt) {
        addFeaturePoint(new XYZPoint(rt, mzIntDp.x, mzIntDp.y));
    }
     ...
     */
    @Override
    public void addFeaturePoint(double mz, XYPoint rtIntDp) {
        addFeaturePoint(new XYZPoint(rtIntDp.x, mz, rtIntDp.y));
    }
    ...
    /**
     * Closes the feature and adds a final data point with the minimum intensity.
     *
     * @param mzIntDp   the last mz intensity data pair
     * @param closingRt the retention time
     */
    @Override
    public void closeFeature(XYPoint mzIntDp, double closingRt) {
        addFeaturePoint(new YMinPoint(mzIntDp.x), closingRt);
        closeFeature();
    }
     ...
     */
    @Override
    public void closeFeature(double closingRt) {
        addFeaturePoint(new XYPoint(data.get(data.size() - 1).y, Constants.MIN_ABUNDANCE), closingRt);
        closeFeature();
    }

    /**
     * Closes the feature and caluclates the feature stats.
     */
    @Override
     ...
     */
    @Override
    public Map<Constants.MSN, Set<Integer>> getMsnScans(ScanContainer container, Range timeRange) {
    ...
    public void addMsnSpectrum(Constants.MSN msn, FeatureSet featureSet) {
        msnManager.addMsnSpectrum(msn, featureSet);
    }
    ...
    public boolean equals(Object obj) {
        ...
        FeatureImpl profile = (FeatureImpl) obj;
