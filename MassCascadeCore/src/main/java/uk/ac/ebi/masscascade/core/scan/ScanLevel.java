...
/**
 * Class describing one MSn level in the scan file.
 */
public class ScanLevel {
    ...
    public static class Builder {
         ...
         */
        public ScanLevel build() {

            return new ScanLevel(this);
        }
    }
     ...
     */
    private ScanLevel(Builder builder) {
     ...
     *
     * @param aScanFileLevel the MSn level to be compared to
     * @return boolean if the information is identical
     */
    @Override
    public boolean equals(Object aScanFileLevel) {

        if (this == aScanFileLevel) return true;

        if (!(aScanFileLevel instanceof ScanLevel)) return false;

        ScanLevel scanLevel = (ScanLevel) aScanFileLevel;

        return this.getIonMode().equals(scanLevel.getIonMode()) &&
                this.getMsn() == scanLevel.getMsn() &&
                this.getMzRange().equals(scanLevel.mzRange) &&
                this.getScanRange().equals(scanLevel.scanRange);
    }
